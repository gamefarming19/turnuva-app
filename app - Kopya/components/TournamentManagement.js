...
const exportMatchesToExcel = () => {
    if (matches.length === 0) return Swal.fire("Hata", "Henüz oynanmış maç yok.", "error");

    const data = matches
      .sort((a, b) => a.round - b.round || a.tableNumber - b.tableNumber)
      .map((m) => {
        // 🚨 KRİTİK DÜZELTME: Maç bittiyse o turun rengini geçmişin başına ekle
        const fullColorH1 = m.status === 'completed' ? "W" + (m.p1_colorHistory || "") : (m.p1_colorHistory || "");
        const fullColorH2 = m.status === 'completed' ? "B" + (m.p2_colorHistory || "") : (m.p2_colorHistory || "");

        const row = {
          "TUR": m.round,
          "MASA": m.tableNumber === 99 ? "BYE" : m.tableNumber,
          "OYUNCU 1": m.p1,
          "RENK1": fullColorH1, // Artık son maçı da içerir
          "SONUÇ": m.status === 'completed' ? m.result : "Oynanıyor",
          "RENK2": fullColorH2, // Artık son maçı da içerir
          "OYUNCU 2": m.p2,
          "HAKEM": m.details?.refereeName || "-",
          "BİTİŞ SAATİ": m.details?.finishTime || "-"
        };

        selectedT.customFields?.forEach(field => {
            row[`P1 ${field.label}`] = m.details?.[`p1_${field.label}`] || 0;
            row[`P2 ${field.label}`] = m.details?.[`p2_${field.label}`] || 0;
        });
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Maç Sonuçları");
    XLSX.writeFile(workbook, `${selectedT.name.replace(/\s/g, "_")}_Sonuclar.xlsx`);
  };
  return (
...
<button onClick={exportMatchesToExcel} className="bg-indigo-600 text-white px-8 py-4 rounded-[2rem] font-black shadow-xl uppercase tracking-widest text-xs flex items-center gap-2">
    {isDemo && <Lock size={14}/>} İNDİR
</button>  
 </div>
                ...
                <table className="w-full text-xs border-collapse">
                    <thead>
                        <tr className="bg-slate-100 text-slate-500 uppercase font-black tracking-widest text-center border">
                            <th className="p-4 border">SIRA</th>
                            <th className="p-4 border">B.NO</th>
                            <th className="p-4 border text-left">AD SOYAD</th>
                            <th className="p-4 border text-left">KURUM</th>
                            <th className="p-4 border bg-indigo-50 text-indigo-600 text-lg">PUAN</th>
                            <th className="p-4 border text-blue-500 italic">BH:GP/C1</th>
                            <th className="p-4 border text-slate-400 font-medium">SB</th>
                            <th className="p-4 border text-slate-400 font-medium">BHs</th>
                            <th className="p-4 border">WIN/P</th>
                            <th className="p-4 border text-slate-400">RENK (DENGESİ)</th>
                            <th className="p-4 border text-red-400">SİL</th>
                        </tr>
                    </thead>
                    <tbody className="font-bold text-slate-700 text-center uppercase">
                        {calculatedPlayers.map((p, i) => (
                            <tr key={p.id} className="hover:bg-slate-50 border-b last:border-0 transition-colors">
                                <td className="p-4 font-black text-lg">{i + 1}</td>
                                <td className="p-4 opacity-40">{p.bNo}</td>
                                <td className="p-4 text-left font-black text-sm truncate max-w-[200px]">{p.name}</td>
                                <td className="p-4 text-left text-[9px] text-slate-400">{p.school || '-'}</td><td className="p-4 text-indigo-600 bg-indigo-50/20 text-lg font-black">{p.points}</td>
                                <td className="p-4 text-blue-500">{p.bh_c1.toFixed(1)}</td>
                                <td className="p-4 opacity-60 font-medium">{p.sb.toFixed(1)}</td>
                                <td className="p-4 opacity-60 font-medium">{p.bh_sum.toFixed(1)}</td>
                                <td className="p-4 font-medium">{p.winCount}</td>
                                <td className="p-4 border text-[10px]">
  <span className={Math.abs((p.whiteCount||0)-(p.blackCount||0)) >= 2 ? 'text-red-500 font-black' : 'text-slate-400'}>
    {(p.whiteCount||0) - (p.blackCount||0) > 0 ? '+' : ''}{(p.whiteCount||0) - (p.blackCount||0)}
  </span>
  <span className="ml-1 opacity-40 font-mono">({p.colorHistory || '-'})</span>
</td>
                                <td className="p-4 text-center"><button onClick={async()=> {if(confirm('Sil?')) await deleteDoc(doc(db,"players",p.id))}}>
                                <Trash2 size={18} className="text-red-200 hover:text-red-500 mx-auto"/></button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>

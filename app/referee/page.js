...
import { 
  collection, onSnapshot,arrayUnion, query, where, doc, 
...
const handleResult = async (m, score, wName) => {
  ...
  const res = await Swal.fire({ 
    title: score === "0.5-0.5" ? "Berabere mi?" : `${wName} Kazandı mı?`, 
    icon: 'question', 
  ...
  if (res.isConfirmed) {
    try {
      ...
      await submitMatchResult({
        ...
        refereeId: auth.currentUser.uid,
        details: details
      });

      // Görsel güncellemeleri burada yap

...
import { ArrowRight, Trash2, Globe, Check, ChevronLeft, Mail, Clock, ShieldCheck, Lock, AlertCircle } from "lucide-react";
...
export default function AdminPage() {
  ...
  // 1. OTURUM KONTROLÜ VE SİSTEM KİLİDİ (GİRİŞTE BİR KEZ ÇALIŞIR)
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, async (u) => {
      if (u) {
        ...
        // 🔐 KİLİT ÇÖZÜMÜ: Sistemi girişte bir kez aktif et, onSnapshot içine koyma!
        try {
            const initialSnap = await getDoc(userRef);
            if (initialSnap.exists() && initialSnap.data().role === "admin") {
                await updateDoc(userRef, { systemActive: true });
            }
        } catch (e) { console.error("Kilit açma hatası:", e); }
        ...
        const unsubUser = onSnapshot(userRef, (snap) => {
          if (snap.exists()) {
            ...
            if (userData.role === "superadmin") {
              ...
              setIsDemo(true);
              updateDoc(userRef, { isLicensed: false });
            } else {
              setLicenseStatus("active");
              setIsDemo(false);
            }
  ...
  // 3. GÜVENLİ ÇIKIŞ (HAKEMLERİ KİLİTLER)
  const handleLogoutWithLock = async () => {
    if (user?.uid) {
      try {
        const userRef = doc(db, "users", user.uid);
        // 🔒 ÇIKIŞ YAPILINCA SİSTEMİ KAPATIR
  ...
  const handlePortalCodeUpdate = async (newCode) => {
    ...
    } catch (e) { Swal.fire("Hata", "Kod güncellenemedi.", "error"); }
  };
  ...
  // 🛡️ GÜVENLİK EKRANLARI
  if (licenseStatus === "unverified") return <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center"><div className="bg-slate-900 p-12 rounded-[4rem] border border-slate-800 max-w-md shadow-2xl animate-in zoom-in"><div className="bg-indigo-600/10 w-20 h-20 rounded-full flex items-center justify-center text-indigo-500 mx-auto mb-6"><Mail size={40}/></div><h1 className="text-2xl font-black text-white uppercase italic">Mail Onayı Bekleniyor</h1><p className="text-slate-400 mt-4 text-sm font-medium italic">Onay linkine tıklayıp sayfayı yenileyin.</p><button onClick={() => window.location.reload()} className="mt-8 w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-lg">SAYFAYI YENİLE</button><button onClick={() => signOut(auth)} className="mt-4 text-slate-600 text-xs font-bold uppercase underline">Çıkış Yap</button></div></div>;
  if (licenseStatus === "expired") return <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center font-sans"><div className="bg-slate-900 p-12 rounded-[4rem] border border-slate-800 max-w-md shadow-2xl animate-in zoom-in"><div className="bg-red-500/10 w-20 h-20 rounded-full flex items-center justify-center text-red-500 mx-auto mb-6"><Clock size={40}/></div><h1 className="text-2xl font-black text-white uppercase tracking-tighter italic text-red-500">Lisans Süreniz Doldu</h1><p className="text-slate-400 mt-4 text-sm font-medium italic leading-relaxed">Sistemi kullanmaya devam etmek için lütfen yönetici Ali Yahşi ile iletişime geçin.</p><div className="mt-8 bg-slate-950 p-4 rounded-2xl text-[9px] font-mono text-indigo-400 break-all border border-white/5 opacity-50 underline uppercase">SİSTEM ID: {auth.currentUser?.uid}</div><button onClick={handleLogoutWithLock} className="mt-8 text-slate-600 font-bold text-xs uppercase underline tracking-widest">Giriş Sayfasına Dön</button></div></div>;
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        {activeTab === 'portal' && (
          <div className="max-w-4xl animate-in fade-in text-left">
            ...
            <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-200 text-left">
              <div className="flex items-center gap-4 mb-8 bg-indigo-50 p-6 rounded-[2rem] border border-indigo-100">
                ...
                <div>
                  ...
                  <p className="text-indigo-400 text-xs font-bold mt-1 uppercase italic tracking-tighter">Bu kod, tüm turnuvalarınızın anahtarıdır.</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4 text-left">
                  ...
                  <input defaultValue={user?.spectatorCode || ""} onBlur={(e) => handlePortalCodeUpdate(e.target.value)} placeholder="Örn: denizli20" className="w-full bg-slate-50 p-6 rounded-3xl font-black text-2xl text-indigo-600 outline-none border-2 border-transparent focus:border-indigo-500 transition-all shadow-inner" />
                ...
                <div className="space-y-4 text-left">
                   ...
                   <div className="bg-emerald-50 p-6 rounded-[2.5rem] border border-emerald-100 flex flex-col justify-between h-[120px]">
                      <p className="text-[10px] font-bold text-emerald-700 truncate opacity-60 uppercase mb-2">{typeof window !== "undefined" ? `${window.location.host}/portal/${user?.spectatorCode || "..."}` : ""}</p>
                      <button onClick={() => { if(!user?.spectatorCode) return Swal.fire("Uyarı", "Önce bir kod belirleyin", "warning"); navigator.clipboard.writeText(`${window.location.origin}/portal/${user.spectatorCode}`); Swal.fire({ title: "Kopyalandı!", icon: "success", timer: 800, showConfirmButton: false }); }} className="w-full bg-white text-emerald-600 py-3 rounded-2xl font-black text-xs shadow-sm hover:bg-emerald-600 hover:text-white transition-all uppercase tracking-widest shadow-emerald-200/50">Linki Kopyala</button>
                   </div>
                </div>
              </div>
            </div>
        ...
        {/* DİĞER MODÜLLER - ⚠️ KISITLAMA VERİLERİ AKTARILIYOR */}
        {activeTab === 'create' && (
            <CreateTournament 
                user={user} 
